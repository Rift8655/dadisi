...
export default function MyRsvpsPage() {
  ...
  const getRsvpStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "waitlisted":
        return <Badge className="bg-orange-500/10 text-orange-600">Waitlisted</Badge>
      case "attended":
        return <Badge className="bg-purple-500/10 text-purple-600">Attended</Badge>
      case "cancelled":
  ...
  const filteredRegistrations = registrations.filter((reg) => {
    ...
    if (filter === "upcoming") return reg.eventStatus === "upcoming" || reg.eventStatus === "ongoing"
    if (filter === "past") return reg.eventStatus === "past"
    return true
  })

  const activeRegistrations = registrations.filter(r => r.status !== "cancelled")
  const upcomingCount = activeRegistrations.filter(r => r.eventStatus === "upcoming" || r.eventStatus === "ongoing").length
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">Events you&apos;ve checked into</p>
            </CardContent>
          </Card>
        </div>
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="flex-shrink-0 bg-primary/5 p-6 text-center md:w-32">
                      <p className="text-sm font-medium text-muted-foreground">
                        {new Date(reg.event.starts_at).toLocaleDateString("en-US", { month: "short" })}
                      </p>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1">
                              ...
                              {reg.event.is_online ? "Online Event" : reg.event.venue || "TBA"}
                            </span>
                            <span className="flex items-center gap-1">
                              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 002 2 2 2 0 012 2v3a2 2 0 01-2 2M19 5a2 2 0 012 2v3a2 2 0 01-2 2 2 2 0 00-2 2v3a2 2 0 002 2" />
                              </svg>
                              {reg.ticket.name}
                              {reg.ticket.price > 0 && ` (${formatCurrency(reg.ticket.price, reg.ticket.currency)})`}
                            </span>
                          </div>
                          <p className="mt-2 text-xs text-muted-foreground">
                            Confirmation: <code className="bg-muted px-1.5 py-0.5 rounded">{reg.confirmation_code}</code>
                          </p>
                        </div>

                        {/* Actions */}
                        <div className="flex gap-2">
                          {reg.status === "confirmed" && reg.eventStatus !== "past" && (
                            <Button 
                              variant="outline" 
                              size="sm"
                              onClick={() => {
                          ...
                          <Button variant="outline" size="sm" asChild>
                            <Link href={`/events/${reg.event.id}`}>View Event</Link>
                          </Button>
                        </div>
                      </div>
                    </div>
