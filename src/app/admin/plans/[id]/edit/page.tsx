...
import { usePlan, useUpdatePlan } from "@/hooks/useAdminPlans"
...
export default function EditPlanPage() {
  ...
  const { data: planData, isLoading: loadingPlan } = usePlan(planId)
  ...
  useEffect(() => {
    if (planData) {
      ...
      // Map system features from the dedicated key in the API response
      const systemFeatures: SystemFeatureInput[] = Array.isArray(
        plan.system_features
      )
        ? plan.system_features
            .filter((sf: any) => sf.is_set || sf.value !== undefined) // Only set those with values
            .map((sf: any) => ({
              id: sf.id,
      ...
      if (typeof plan.description === "object" && plan.description !== null) {
        descriptionVal = plan.description.en || ""
      } else if (typeof plan.description === "string") {
        descriptionVal = plan.description
      }

      // Extract price - prefer base_monthly_price, fallback to price
      const price =
        typeof plan.base_monthly_price === "number"
          ? plan.base_monthly_price
          : typeof plan.price === "number"
            ? plan.price
            : Number(plan.base_monthly_price || plan.price || 0)
      ...
      form.reset({
        ...
        requires_student_approval: plan.requires_student_approval ?? false,
        ...
        display_features: Array.isArray(plan.display_features)
          ...
          : Array.isArray(plan.features)
            ...
            : [],
        monthly_promotion:
          plan.monthly_promotion || plan.promotions?.monthly || null,
        yearly_promotion:
          plan.yearly_promotion || plan.promotions?.yearly || null,
      })
    }
  }, [planData, form])
  ...
  const handleSubmit = async (values: AdminPlanFormValues) => {
    try {
      await updateMut.mutateAsync({ id: planId, payload: values })
  ...
  if (loadingPlan) {
    return (
      <AdminDashboardShell title="Edit Plan">
          ...
          <Card>
            ...
            <CardContent>
              <Alert className="mb-4">
                <AlertDescription>
                  💡 <strong>Note:</strong> Use System Features above to control
                  actual user access and quotas. These Display Features are only
