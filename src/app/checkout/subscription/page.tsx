...
export default function SubscriptionCheckoutPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const error = err as { message?: string; data?: { error_code?: string; action?: string } }
      ...
      if (errorCode === "PAYMENT_PENDING") {
        showWarning("You have a pending payment. Please complete or cancel it first.")
        router.push("/dashboard/subscription")
        return
      }
  ...
  return (
    <div className="container mx-auto py-12 px-4 max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <Button variant="ghost" onClick={() => router.push("/dashboard/subscription")}>
          <ArrowLeft className="mr-2 h-4 w-4" />
          Back to Subscription
        </Button>
