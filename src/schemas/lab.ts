...
// Enums
export const LabSpaceTypeSchema = z.enum(['wet_lab', 'dry_lab', 'greenhouse', 'mobile_lab']);
export const LabBookingStatusSchema = z.enum(['pending', 'approved', 'rejected', 'cancelled', 'completed', 'no_show']);
...
export const LabBookingSchema = z.object({
  ...
  recurrence_parent_id: z.number().nullable(),
...
export const CreateLabBookingSchema = z.object({
  lab_space_id: z.number().min(1, 'Please select a lab space'),
...
export const LabAvailabilityResponseSchema = z.object({
  success: z.boolean(),
  data: z.object({
    space: LabSpaceSchema.pick({ id: true, name: true, slug: true, type: true, capacity: true }),
